...
;; * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
;; Using defparameter instead of defconstant to
...
(defun pat-match-2 (pattern input &optional (bindings no-bindings))
...
(defun eliza ()
  ...
  (loop
    (print 'eliza>)
    (write (c:flatten (use-eliza-rules (read))) :pretty t)))
            ...
            (let ((result (pat-match (rule-pattern rule) input)))
              (if (not (eq result fail))
                  (sublis (switch-viewpoint result)
                          (c:random-elt (rule-responses rule))))))
        *eliza-rules*))
