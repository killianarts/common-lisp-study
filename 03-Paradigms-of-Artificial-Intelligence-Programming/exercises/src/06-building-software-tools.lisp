...
(defparameter no-bindings '((t . t)) "Indicates pat-match success, with no variables")
...
(defun eliza ()
  ...
  ;; Original used 'eliza> instead of "Eliza> ".
  (interactive-interpreter "Eliza> "
                           (c:compose #'c:flatten #'use-eliza-rules)))
...
(defun interactive-interpreter (prompt transformer)
  ...
  (loop
    (handler-case
        (progn
          (if (stringp prompt)
              ;; NOTE: I have slightly altered the original code.
              ;; Original: (print prompt)
              (princ prompt)
              ;; NOTE: This funcall produces an infinite loop error when prompt is 'eliza>.
              (funcall prompt))
...
(defun is-1 (value) #'(lambda (x) (eql x value)))
